...
import { useLayers } from 'entities/theme/config/layerContext'
...
export const GalleryMerch = () => {
    ...
    const { registerLayer } = useLayers()

    useEffect(() => {
        registerLayer('GalleryMerch', 1001) // Приоритет выше чем у меню
        return () => registerLayer('GalleryMerch', 0)
    }, [registerLayer])
    //   переписать на экспорт из того массива или на стор!
    // разобраться с адаптивностью фоток под мобильную версию!
